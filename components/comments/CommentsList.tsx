...
const CommentsList: React.FC<CommentsListProps> = ({ topicId, addComment }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = db.collection('qran_comments')
            ...
            .onSnapshot(snapshot => {
                ...
                // Simplified logic: Only set the documents for the current topic.
                // This avoids the permission error for fetching admin actions for regular users.
                setAllDocs(fetchedDocs);
                setLoading(false);

            }, error => {
                console.error("Error fetching comments for topic. This may require a Firestore index.", error);
                setLoading(false);
            });
    ...
    const { visibleComments, repliesMap } = useMemo(() => {
        ...
        // Since we no longer fetch admin actions for non-admins, this array will be empty for them.
